...
class Indiv:
    ...
    @staticmethod
    def from_haploids(haps: list[Haploid], name: str = None, allosome_labels: list[str] = []):
        """
    ...
    def __init__(self, Ls:list[float]|None=None, label:str="POP", fname:str|None=None, 
                 labs: tuple[str, str]=("A", "_B"), selectchrom: str|None=None, chroms: list[Chropair] | None = None, 
        ...
        selectchrom: list of integers
            Default is `None`. This argument is forwarded as-is to haploid.from_file. It acts as a filter on the chromosomes to load. The default value of
        ...
        methods :func:`~tracts.indiv.Indiv.from_files` or :func:`~tracts.indiv.Indiv.from_haploids`.
        """
        if fname is None:
            self.Ls = Ls
            if name:    
