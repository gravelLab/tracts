...
class PhaseTypeDistribution(ABC):
    ...
    def tractlength_histogram(self, population_number: int, bins: npt.ArrayLike, density=False) -> npt.ArrayLike:
        ...
        bins: npt.ArrayLike
            A point grid on (0, Inf) where the CDF or density have to be evaluated.
...
class PhTMonoecious(PhaseTypeDistribution):
    ...
    def PhT_CDF(self, x, population_number, s1=None) -> npt.ArrayLike:
        ...
        The PhT parameters (initial state, transition matrix) are taken from a PhTMonoecious
    ...
    def tractlength_histogram_windowed(self, population_number: int, bins: npt.ArrayLike, L: float,
                                       exp_Sx_per_bin: npt.ArrayLike = None, density=False, freq=False) -> npt.ArrayLike:
        ...
        density: bool, default False
            If True, computes the PhT density values evaluated on the grid. Else, returns the histogram values on the grid.
    ...
    def PhT_density_windowed(self, population_number, S, alpha, S0_inv, bins: npt.ArrayLike, L: float, s1=None,
        ...
        The PhT parameters (initial state, transition matrix) are taken from a PhTMonoecious object 
        ...
        bins: npt.ArrayLike
            A point grid on (0, L) where the density has to be evaluated.  
        L: float
            The length of the finite chromosome.
        s1 :
            Not used in the Monoecious model.
        ...
        Notes
        -------     
        The code truncates bins to the interval [0,L] and adds the point L if it is not included in bins.
        ...
        **Don't run** this function directly. To get a PhT density on a finite chromosome,
        use `tractlength_histogram_windowed` setting `density=True`.
        """
...
class PhTDioecious(PhaseTypeDistribution):
    ...
    def __init__(self, migration_matrix_f, migration_matrix_m, rho_f, rho_m, X_chromosome=False,
        ...
        self.migration_matrix_f_unchanged = migration_matrix_f.copy() # For testing purposes
    ...
    def PhT_density(self, x, population_number, s1=None):
        ...
        """
        Computes a Phase-Type density at a given point x in (0, infinity).
        The PhT parameters (initial state, transition matrix) are taken from a
        PhTDioecious object together with the specification of a population of interest.
    ...
    def tractlength_histogram_windowed(self, population_number: int, bins: npt.ArrayLike, L: float,
        ...
        using the PhTDioecious admixture model.
        ...
        exp_Sx_per_bin_f: npt.ArrayLike, default None
            The precomputed values of e^(S*x) for every x in bins, for the maternally inherited alleles. Used internally to speed up computation.
        exp_Sx_per_bin_m: npt.ArrayLike, default None
            The precomputed values of e^(S*x) for every x in bins, for the paternally inherited alleles. Used internally to speed up computation.
        density: bool, default False
            If True, computes the PhT density values evaluated on the grid. Else, returns the histogram values on the grid.
        freq: bool, default False
            If density is True, whether to return density on the frequency scale.
            If True, the density values are scaled so that their integral over (0,L) is equal to the expected number of tracts on (0,L).
            If False, the density values integrate to 1 over (0,L).
        return_only: int, default None
            For internal use only, to manage the combination of maternally and paternally inherited fracts. If set to 0 (resp. 1), only paternally (resp. maternally) inherited tracts are considered.
        ...
        Notes
        -----   
        When density=True, the first returned argument is a transformed version of the input bins grid. This is because the density is defined on the finite chromosome [0,L] as a mixture of a density
        with support on (0,L) and point masses at 0 and L. The returned bins grid removes the points 0 and L if they were included in the input bins grid,
