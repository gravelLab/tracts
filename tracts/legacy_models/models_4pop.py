...
def pppp_pxxx_xpxx_xxpx(*params):
    """ A simple model in which populations 1, 2, 3 and 4 arrive discretely at first
        ...
        """
    # Added t4, t5"
    (prop1, tstart, prop3, prop4, t3, t4, t5, prop1_secondary, prop2_secondary, prop3_secondary) = params[0]
    n_pops = 4
    tstart *= 100
    t3 *= 100
    t4 *= 100
    ...
    # some sanity checks
    if t3 > tstart or t3 < 0 or tstart < 0 or t4 < 0 or t4 > tstart or t5 < 0 or t5 > tstart:
        # Added t4 and t5 sanity checks"
    ...
    prop1_ceil = (prop1_secondary - prop1_floor) / (
            1 - prop1_floor)  # Using only prop1 here as migrants from one population (San) only
    ...
    prop2_ceil = (prop2_secondary - prop2_floor) / (
            1 - prop2_floor)  # Using only prop2 here as migrants from one population (Bantu) only
    ...
    prop3_ceil = (prop3_secondary - prop3_floor) / (
            1 - prop3_floor)  # Using only prop3 here as migrants from one population (European) only
...
def outofbounds_pppp_pxxx_xpxx_xxpx(*params):  # I did not change much here
    ...
    (prop1, tstart, prop3, prop4, t3, t4, t5, prop1_secondary, prop2_secondary, prop3_secondary) = params[
        0]  # Added t4 t5"
...
def pppp_pxxx_xpxx_xxpx_fix(params, fracs):
    ...
    def fun(params):
        ...
        return propfrommig(pppp_pxxx_xpxx_xxpx(
            (prop1, tstart, prop3, prop4, t3, t4, t5, prop1_secondary, prop2_secondary, prop3_secondary)))[
               0:n_pops - 1] - fracs[0:n_pops - 1]  # Added t4, t5"
    ...
    # parameter values.
    (prop1, prop3, prop4) = scipy.optimize.fsolve(fun, (
        .3, .2, .2))  # Added .3 here for pop1 (San) as we now have 4-way admixture
    return pppp_pxxx_xpxx_xxpx(
        (prop1, tstart, prop3, prop4, t3, t4, t5, prop1_secondary, prop2_secondary, prop3_secondary))  # Added t4, t5"
...
def outofbounds_pppp_pxxx_xpxx_xxpx_fix(params, fracs):
    ...
    return outofbounds_pppp_pxxx_xpxx_xxpx(
        (prop1, tstart, prop3, prop4, t3, t4, t5, prop1_secondary, prop2_secondary, prop3_secondary))  # Added t4, t5"
