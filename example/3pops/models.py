...
"""a simple model in which populations 1 and 2 arrive discretely at first generation, 3 at a subsequent generation. 
	. If a time is not integer, the migration 
	is divided between neighboring times proportional to the non-integer time fraction. 
	We'll assume population 3 still
	replaces migrants from 1 and 2 after the replacement from population 1 and 2 if they arrive at same generation."""
...
def ppx_xxp(*params):	
	...
	In this prop1 is the initial proportion from population 1, prop2=1-prop1, 
	tstart is the arrival times of pops (1,2) 
	t3 is the arrival time of pop 3
	...
	gen=int(numpy.ceil(tstart))+1 #how many generations we'll need to accomodate all the migrations. The +1 is not really necessary here.
	timefrac=gen-tstart-1 #how far off the continuous time is from its discrete optimization
	mig=numpy.zeros((gen+1,3))#Build an empty matrix with enough generations to handle all migrations
	
	#replace a fraction at first and second generation to ensure a continuous model
	prop2=1-prop1
	...
	#we want the total proportion replaced  by 3 to be prop3. We therefore add a fraction 
	#f at generation gen-1, and (prop3-f)/(1-f) at generation gen.
...
#We don't have to calculate all the tract length distributions to have the global ancestry proportion right. Here we define a function that automatically adjusts the migration rates to have the proper global ancestry proportion, saving a lot of optimization time!
#We first define a function that calculates the final proportions of ancestry based on the migration matrix
def propfrommig(mig):
	curr=mig[-1,:]
	for row in mig[-2::-1,:]:
...
def ppx_xxp_fix((tstart,t3),fracs):
	...
	#an auxiliary function that, given the "missing" parameters, returns the full migration matrix
	def fun((prop3,prop1)):
