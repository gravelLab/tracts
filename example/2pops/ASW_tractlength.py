...
import sys
sys.path.append("../../") #path to tracts, may have to adjust if the file is moved
import tracts
import pp,pp_px
import numpy,pylab
...
for i in range(rep_pp):
	xopt=tracts.optimize_cob(startrand,bins,Ls,data,nind,func,outofbounds_fun=bound,cutoff=cutoff,epsilon=1e-2)
	try:
...
for i in range(0,rep_pp_px):
	xopt2=tracts.optimize_cob(startrand2,bins,Ls,data,nind,func2,outofbounds_fun=bound2,cutoff=cutoff,epsilon=1e-2)
	try:
